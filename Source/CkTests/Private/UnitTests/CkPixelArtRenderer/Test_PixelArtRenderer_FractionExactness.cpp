// C++ unit test for the pixel-art renderer's screen-percentage fraction (CkPixelArtRender).
//
...
//
// Surface in Session Frontend: CkTests.UnitTests.CkPixelArtRender.Fraction.<scenario>
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_PixelArtRender_FractionExactness,
    "CkTests.UnitTests.CkPixelArtRender.Fraction.LandsOnTargetWidth",
    kCkUnitTestFlags)

bool FCkTest_PixelArtRender_FractionExactness::RunTest(const FString& Parameters)
{
    ...
    for (const auto ViewportWidth : ViewportWidths)
    {
        ...
        {
            const auto Fraction = UCk_Utils_PixelArtRender_UE::Get_ExactFraction(TargetWidth, ViewportWidth);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_PixelArtRender_FractionRejectsDegenerateInput,
    "CkTests.UnitTests.CkPixelArtRender.Fraction.RejectsDegenerateInput",
    kCkUnitTestFlags)

bool FCkTest_PixelArtRender_FractionRejectsDegenerateInput::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("zero viewport width falls back to native resolution"),
        UCk_Utils_PixelArtRender_UE::Get_ExactFraction(640, 0), 1.0f);

    TestEqual(TEXT("negative viewport width falls back to native resolution"),
        UCk_Utils_PixelArtRender_UE::Get_ExactFraction(640, -1920), 1.0f);

    TestEqual(TEXT("non-positive target width falls back to native resolution"),
        UCk_Utils_PixelArtRender_UE::Get_ExactFraction(0, 1920), 1.0f);
    ...
    TestEqual(TEXT("target wider than the viewport falls back to native resolution"),
        UCk_Utils_PixelArtRender_UE::Get_ExactFraction(2560, 1920), 1.0f);
