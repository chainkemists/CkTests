// C++ unit test for the pixel-art renderer's per-world config registry (CkPixelArtRender).
//
...
//
// Surface in Session Frontend: CkTests.UnitTests.CkPixelArtRender.ConfigRegistry.<scenario>
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_PixelArtRender_ConfigRoundtrip,
    "CkTests.UnitTests.CkPixelArtRender.ConfigRegistry.Roundtrip",
    kCkUnitTestFlags)

bool FCkTest_PixelArtRender_ConfigRoundtrip::RunTest(const FString& Parameters)
{
    ...
    TestFalse(TEXT("an unconfigured world reads as unset"),
        FCk_PixelArtRender_StateRegistry::TryGet(World).IsSet());

    const auto Written = ck_test_pixel_art_config_roundtrip::MakeConfig(180, 3);
    FCk_PixelArtRender_StateRegistry::Set(World, Written);

    const auto ReadBack = FCk_PixelArtRender_StateRegistry::TryGet(World);
    ...
    const auto Replacement = ck_test_pixel_art_config_roundtrip::MakeConfig(360, 2);
    FCk_PixelArtRender_StateRegistry::Set(World, Replacement);

    const auto Replaced = FCk_PixelArtRender_StateRegistry::TryGet(World);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_PixelArtRender_ConfigIsPerWorld,
    "CkTests.UnitTests.CkPixelArtRender.ConfigRegistry.IsPerWorld",
    kCkUnitTestFlags)

bool FCkTest_PixelArtRender_ConfigIsPerWorld::RunTest(const FString& Parameters)
{
    ...
    ON_SCOPE_EXIT
    {
        FCk_PixelArtRender_StateRegistry::Clear(WorldA);
        FCk_PixelArtRender_StateRegistry::Clear(WorldB);
        WorldA->DestroyWorld(false);
        WorldB->DestroyWorld(false);
    };

    FCk_PixelArtRender_StateRegistry::Set(WorldA, ck_test_pixel_art_config_roundtrip::MakeConfig(180, 3));

    // PIE runs several worlds at once; configuring one must never pixelate the others.
    TestTrue (TEXT("the configured world reads as set"),   FCk_PixelArtRender_StateRegistry::TryGet(WorldA).IsSet());
    TestFalse(TEXT("the other world is still unset"),      FCk_PixelArtRender_StateRegistry::TryGet(WorldB).IsSet());

    FCk_PixelArtRender_StateRegistry::Clear(WorldA);

    TestFalse(TEXT("clearing one world leaves it unset"),  FCk_PixelArtRender_StateRegistry::TryGet(WorldA).IsSet());
    TestFalse(TEXT("clearing one world leaves the other unset"),
        FCk_PixelArtRender_StateRegistry::TryGet(WorldB).IsSet());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_PixelArtRender_ConfigRejectsNullWorld,
    "CkTests.UnitTests.CkPixelArtRender.ConfigRegistry.RejectsNullWorld",
    kCkUnitTestFlags)

bool FCkTest_PixelArtRender_ConfigRejectsNullWorld::RunTest(const FString& Parameters)
{
    ...
    TestFalse(TEXT("a null world reads as unset"),
        FCk_PixelArtRender_StateRegistry::TryGet(nullptr).IsSet());
