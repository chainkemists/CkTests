...
| P2 | Input + widget substrate | Menu input-layer entity (catch-all Consume open/close), Tab global action, empty Slate switchboard shell on the viewport driven by it | ⏳ Pending |
| P2.5 | Gameplay onto the stack | Pawn movement via low-priority layer; control panel rows via mid-priority layer; masking verified | ⏳ Pending |
| P3 | Group Rail layout (default) | Full default layout: groups, filter, navigate, travel. **First user-test checkpoint — STOP for PIE verification** | ⏳ Pending |
