...
// a MinMax-bounded integer attribute exposes three separately-bindable signals
...
class UCk_AutoTest_Attribute_IntegerMinMaxComponents : UCk_AutoTest_Base
{
    private FCk_Handle_IntegerAttribute _Power;
    ...
    void DoBeginPlay(FCk_Handle InHandle)
    {
        ...
        _Power = utils_integer_attribute::Add(LocalHandle, Params);
    ...
    {
        _Step = 1;
        utils_integer_attribute::Request_Override(_Power, 25, ECk_MinMaxCurrent::Min);
    }

    private void Step2_OverrideMax()
    {
        _Step = 2;
        utils_integer_attribute::Request_Override(_Power, 175, ECk_MinMaxCurrent::Max);
    }

    UFUNCTION()
    private void OnMinChanged(FCk_Handle InOwner, FCk_Payload_IntegerAttribute_OnValueChanged InPayload)
    {
        ...
        if (_Step == 1)
        {
            ...
            Assert_Equals_Int(utils_integer_attribute::Get_BaseValue(_Power, ECk_MinMaxCurrent::Min), 25,
    ...
    UFUNCTION()
    private void OnMaxChanged(FCk_Handle InOwner, FCk_Payload_IntegerAttribute_OnValueChanged InPayload)
    {
        if (IsFinished()) { return; }
        _MaxFires++;
        ...
        if (_Step == 2)
        {
            ...
            Assert_Equals_Int(utils_integer_attribute::Get_BaseValue(_Power, ECk_MinMaxCurrent::Max), 175,
    ...
    UFUNCTION()
    private void OnCurrentChanged(FCk_Handle InOwner, FCk_Payload_IntegerAttribute_OnValueChanged InPayload)
    {
        _CurrentFires++;
    }
}
