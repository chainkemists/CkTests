...
//============================================================================
//
// Every stockpile / unlock is a boolean flag. Game code can compute numeric
// thresholds externally and project them to these booleans (e.g. setting
// HasEnoughFood when ActualFood >= 500) — the planner only ever sees bools,
// which is what makes classical GOAP tractable.
//
// Actions registered:
//   GatherFood / GatherGold / GatherWood   — each sets its HasEnough flag
//   BuildBarracks                          — needs HasEnoughWood, sets HasBarracks
//   AdvanceToFeudalAge                     — needs Food+Gold+Barracks, sets IsInFeudalAge
// Goal:
//   ReachFeudalAge                         — IsInFeudalAge == true
...
class UCk_EntityScript_GoapGym_Empire : UCk_EntityScript_UE
{
	...
	int32 AutoStep   = 0;
	int32 PlanCount  = 0;
	FString LastGoalName = "(auto)";

	TArray<TSubclassOf<UCk_GoapAction_EntityScript>> LastPlan;
	float LastPlanCost = 0.0f;
	ECk_GoapPlanStatus LastStatus = ECk_GoapPlanStatus::Idle;
	...
	ECk_EntityScript_ConstructionFlow DoConstruct(FCk_Handle& InHandle)
	{
		...
		GoapEntity.AddAction(UCk_GoapTest_Action_AdvanceToFeudalAge);
		GoapEntity.AddGoal  (UCk_GoapTest_Goal_ReachFeudalAge);

		// Seed initial state — everything starts false. GOAP with missing
		// keys already treats them as false; setting explicitly keeps the
		// debugger's world-state panel populated from the first frame.
		utils_goap::Set_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasEnoughFood), false);
		utils_goap::Set_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasEnoughGold), false);
		utils_goap::Set_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasEnoughWood), false);
		utils_goap::Set_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasBarracks),   false);
		utils_goap::Set_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::IsInFeudalAge), false);

		AutoConfig.TotalSteps = 1;
		AutoConfig.Description = "Boolean AoE: gather all resources, build barracks, advance to Feudal.";
		AutoConfig.GlobalAutoCommand    = "Ck_GymGoap_Auto [0/1]";
		AutoConfig.PerStationAutoCommand = "Ck_GymGoap_AutoEmpire";
		AutoConfig.Steps.Add(FCkGym_AutoStep("Plan for ReachFeudalAge", 0, 0));
		AutoConfig.ManualCommands.Add("Ck_GymGoap_Empire_PlanFeudalAge");
		AutoConfig.ManualCommands.Add("Ck_GymGoap_Empire_ResetWorld");

		AutoTimer = gym_auto::Setup(InHandle, this, FCk_Time(5.0f));

		utils_messaging::BindTo_OnBroadcast(InHandle, FCk_Message_GoapGym_Empire_PlanFeudalAge,
			FCk_Delegate_Messaging_OnBroadcast(this, n"OnPlanFeudal"));
		utils_messaging::BindTo_OnBroadcast(InHandle, FCk_Message_GoapGym_Empire_ResetWorld,
	...
	private void RequestPlanFeudal()
	{
		LastGoalName = "ReachFeudalAge";
		GoapEntity.Request_PlanForGoal(UCk_GoapTest_Goal_ReachFeudalAge);
		PlanCount++;
	}
	...
	UFUNCTION() private void OnResetWorld(FCk_Handle InHandle, FGameplayTag InMsg, FInstancedStruct InPayload)
	{
		...
		LastPlan.Reset();
		LastPlanCost = 0.0f;
		LastStatus = ECk_GoapPlanStatus::Idle;
		LastGoalName = "(reset)";
	}
	...
	UFUNCTION() private void OnPlanComplete(FCk_Handle_Goap InHandle, FCk_Goap_Payload_OnPlanComplete InPayload)
	{
		...
		LastStatus = ECk_GoapPlanStatus::PlanFound;
	}
	...
	UFUNCTION() private void OnPlanFailed(FCk_Handle_Goap InHandle, FCk_Goap_Payload_OnPlanFailed InPayload)
	{
		LastPlan.Reset();
		LastPlanCost = 0.0f;
		LastStatus = ECk_GoapPlanStatus::PlanFailed;
	}

	UFUNCTION() private void DisplayTick(FCk_Handle_Timer InTimerHandle, FCk_Chrono InChrono, FCk_Time InDeltaT)
	{
		auto SelfEntity = ck::ToEntity(this);
		auto Text = gym_auto::FormatHeader(AutoConfig, AutoRunning);
		Text = Text + f"Goal: {LastGoalName}\n";
		Text = Text + f"Plans: {PlanCount}    Last cost: {LastPlanCost}    Length: {LastPlan.Num()}\n";
		Text = Text + f"Status: {LastStatus}\n\n-- World State --\n";
		Text = Text + f"  Food={utils_goap::Get_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasEnoughFood))}";
		Text = Text + f"  Gold={utils_goap::Get_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasEnoughGold))}";
		Text = Text + f"  Wood={utils_goap::Get_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasEnoughWood))}\n";
		Text = Text + f"  Barracks={utils_goap::Get_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::HasBarracks))}";
		Text = Text + f"  Feudal={utils_goap::Get_WorldStateValue(GoapEntity, goap_gym_util::T(empire_tags::IsInFeudalAge))}\n";
		Text = Text + "\n-- Plan --\n";
		if (LastPlan.Num() == 0) { Text = Text + "  (empty)\n"; }
		else
		{
			for (int32 i = 0; i < LastPlan.Num(); i++)
			{
				if (ck::IsValid(LastPlan[i]))
				{
					Text = Text + f"  {i + 1}. {LastPlan[i].Get().GetName()}\n";
				}
			}
		}
		Text = Text + gym_auto::FormatAutoAndCommands(AutoConfig, AutoStep, AutoRunning);
		...
		auto Mode = AutoRunning ? "[AUTO]" : "[MANUAL]";
		CkGym_Common::Update_StationDisplay(SelfEntity, f"EMPIRE {Mode}", Text, "");
	}
}
