...
class UCk_AutoTest_VfxExamples_PairStationsSpawn : UCk_AutoTest_Base
{
    ...
    void DoBeginPlay(FCk_Handle InHandle)
    {
        ...
        auto PlaceholderCount = 0;

        for (auto Pair : Pairs)
        {
            auto CkComponent = utils_particles::Spawn_BehaviorAtLocation(
                Pair.BehaviorId, FVector(0, 0, 300), FRotator::ZeroRotator,
                FVector(1.0, 1.0, 1.0), Pair.TextureName);

            Assert_True(ck::IsValid(CkComponent),
                f"the CkParticles side of pair '{Pair.DisplayName}' must spawn a live component");

            if (ck::IsValid(CkComponent)) { Spawned.Add(CkComponent); }

            auto Original = CkVfxExamples::TryLoad_OriginalSystem(Pair);
            ...
            Assert_True(ck::IsValid(OriginalComponent),
                f"the resolved original system for pair '{Pair.DisplayName}' must spawn a live component");

            if (ck::Is_NOT_Valid(OriginalComponent)) { continue; }
            ...
            if (ck::Is_NOT_Valid(Idle)) { continue; }

            Spawned.Add(Idle);

            Assert_True(Idle.IsActive() == false,
                f"a bAutoActivate=false spawn for pair '{Pair.DisplayName}' must start inactive");

            CkVfxExamples::Request_RestartComponent(Idle);

            Assert_True(Idle.IsActive(),
                f"Request_RestartComponent must re-activate the idle original for pair '{Pair.DisplayName}'");
        }

        if (PlaceholderCount > 0)
        {
            ck::Trace(f"[VfxExamples] {PlaceholderCount} pair(s) took the placeholder branch — the original content is not installed");
        }
