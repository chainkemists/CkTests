...
class ACk_UsfGym_PerInstanceShowcase : AActor
{
    ...
    void Request_SetLook(UCkUsf_LookDefinition InLook, int InCount)
    {
        ...
        if (MID == nullptr)
        {
            ck::Warning("❌ CkUsf gym: per-instance MID creation failed — run 'Generate Look Materials' first");
            return;
        }
        ...
        ck::Trace(f"✅ CkUsf gym: per-instance look [{LookName}] applied to {InCount} instances");
    }
}
